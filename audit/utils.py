...
def save_audit_log(instance, user_id, event_type, old_instance=None):
    try:
        ...
        # 2. Si es una creación, opcionalmente podrías loguear todos los campos iniciales
        # o simplemente dejar el encabezado. Por simplicidad, solo logueamos cambios en UPDATES.
...
def compare_and_save_details(audit_log, instance, old_instance):
    ...
    exclude_fields = ['created_at', 'updated_at', 'key_user_created', 'key_user_updated']
    ...
    for field in instance._meta.fields:
        ...
        # Comparar valores (manejar objetos ForeignKey obteniendo su ID o __str__)
        if old_val != new_val:
            # Convertir a string para almacenamiento en TextField
            str_old = str(old_val.id) if isinstance(old_val, models.Model) else str(old_val)
            str_new = str(new_val.id) if isinstance(new_val, models.Model) else str(new_val)
            
            # Si el valor anterior era None, guardar como string vacío o "None"
            if old_val is None: str_old = ""
            if new_val is None: str_new = ""
